...
bool CTransmitClient::Receive(unsigned long lTimeout)
{
	...
	{
		//printf("Receive throw an exception\n");
...
//ResponseCode CTransmitClient::RecvXMLCmdResponse( CData& oResult, unsigned long lTimeout )
{
	try
	{
		...
		if(CmdNum == 7)
		{
			length = (_recvBuffer[2] << 16) + (_recvBuffer[3] << 8) + _recvBuffer[4] + 5 + 5;
        ...
        if (cResult == NULL)
		{
			//printf("RecvXMLCmdResponse cResult is NULL\n");
			return eResponseCodeErrorRecv;
		}
	}
	catch(...)
	{
		...
		//printf("RecvXMLCmdResponse throw an exception\n");
		return eResponseCodeErrorRecv;
	}
...
SVS::RequestParseResult CTransmitClient::Parse( const char * pBuffer, std::size_t bytes )
{
	try
	{
		if (!_hasHeader)
		{
			...
			if(CmdNum == 7)
			{
				_recvedLen = (pBuffer[2] << 16) + (pBuffer[3] << 8) + pBuffer[4] + 5 + 5;
