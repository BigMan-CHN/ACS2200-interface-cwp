	...
	else if (CHNSYS_NET_SEND_OP == cmd_id)
	{
		...
		if(!strcmp(CMethod.c_str(),"GetHostChnInfo"))
		{
			...
			if (StringCmp(str[1],"name"))
			{
				...
				{
					//cJSON_AddItemToObject(_ParamArry[net_send_num],GET_HOST_CONFIG_RES_CH_NCHNNAME,cJSON_CreateString(arg[0]));
				}				
			}
			if (!strcmp(str[1],"rec_current_status"))
			{
				cJSON_AddItemToObject(_ParamArry[net_send_num],GET_HOST_CONFIG_RES_CH_NCHNRECSTA,cJSON_CreateNumber(atoi(arg[0])));
			
			}
			if (!strcmp(str[1],"rec_task_id"))
			{	
				cJSON_AddItemToObject(_ParamArry[net_send_num],GET_HOST_CONFIG_RES_CH_NCHNRECTASKID,cJSON_CreateString(arg[0]));
			}
			 ...
			 if (StringCmp(str[1],"on"))
			{
				...
				if(Param > 13)
				{
					cJSON_AddItemToObject(_ParamArry[num],GET_HOST_CONFIG_RES_CH_NCHNID,cJSON_CreateNumber(num-1));
				}
				else
				{
					//cJSON_AddItemToObject(_ParamArry[net_send_num],GET_HOST_CONFIG_RES_CH_NCHNID,cJSON_CreateNumber(num-1));
				}
				cJSON_AddItemToObject(_ParamArry[net_send_num],GET_HOST_CONFIG_RES_CH_NCHNSTA,cJSON_CreateNumber(atoi(arg[0])));
				
			}
			 
			if (StringCmp(str[1],"url"))
			{	
				cJSON_AddItemToObject(_ParamArry[net_send_num],"Url",cJSON_CreateString(arg[0]));		
			}
			
			if (StringCmp(str[1],"recording"))
			{
				cJSON_AddItemToObject(_ParamArry[net_send_num],"OldRecState",cJSON_CreateNumber(atoi(arg[0])));
			}
			
			if (!strcmp(str[1],"rec_path"))
			{
				cJSON_AddItemToObject(_ParamArry[net_send_num],"OldTaskID",cJSON_CreateString(arg[0]));		
			}
...
bool CAnalyzeParaProc::ConvertSTRING2CJSONGetRecCtrl(CData &oResult, char *cResult, cJSON *videofiles, cJSON *recresult)
{
	int ret = true;
